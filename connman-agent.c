...
static void connman_agent_method_call(
    ...
    p_type = g_variant_get_type_string(params);
    ...
    if (g_strcmp0("(os)", p_type) == 0) {
        GVariant *req_obj = g_variant_get_child_value(params, 0);
        g_variant_get(req_obj, "&o", &req_path);

        req_obj = g_variant_get_child_value(params, 1);
    ...
    if (g_strcmp0("(oa{sv})", p_type) == 0) {
        ...
        GVariant *obj_array = g_variant_get_child_value(params, 1);
    ...
    if (g_strcmp0("RequestInput", method) == 0) {
        g_message("Request Passphrase\n");
        GVariantBuilder builder;
        g_variant_builder_init (&builder, G_VARIANT_TYPE ("(a{sv})"));
        g_variant_builder_open (&builder, G_VARIANT_TYPE ("a{sv}"));
        g_variant_builder_open (&builder, G_VARIANT_TYPE ("{sv}"));
        g_variant_builder_add (&builder, "s", "Passphrase");
        g_variant_builder_add (&builder, "v", g_variant_new_string("1234567890"));
